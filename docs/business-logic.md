...
4. **Статические агрегации** — предрассчитанные при тренинге профили из `static_aggs.json`
5. **Fill-значения** — медианы признаков из `fill_values.json` для заполнения пропусков

Для построения признаков нужно до 288 исторических наблюдений (6 дней), хранящихся в `route_status_history`. **Cold-start fallback:** если у маршрута меньше 24 наблюдений (12 часов), feature engine подмешивает среднее по складу, чтобы выдать прогноз даже на «холодных» маршрутах.

### Champion / Challenger и shadow-модель

`prediction-service` поддерживает параллельную загрузку **shadow-модели** для A/B-сравнения. Её прогнозы сохраняются в `forecasts` отдельной строкой с собственным `model_version`. Это позволяет:

- Гонять challenger'а на реальном трафике без риска для primary
- Считать его метрики тем же `quality_check`-ом, что и primary
- Промоутить shadow → primary автоматически (по streak побед в `scheduler-service`) или вручную через `POST /model/shadow/promote`

`retraining-service` отвечает за полный цикл champion/challenger: на свежих данных тренируется challenger, сравнивается с champion-ом по `combined_score` и при улучшении автоматически загружается в prediction-service как shadow.
...
|----------|---------------------|--------------|----------|
| Вместимость машины | `TRUCK_CAPACITY` | 33 | Сколько ёмкостей помещается в одну машину |
| Базовый буфер | `BUFFER_PCT` | 0.10 (10%) | Запас на ошибку прогноза |
| Адаптивный буфер | `ADAPTIVE_BUFFER` | `false` | Если `true` — буфер плавающий между `MIN_BUFFER_PCT` и `MAX_BUFFER_PCT` в зависимости от неопределённости прогноза |
| Нижняя граница буфера | `MIN_BUFFER_PCT` | 0.05 | Активна при `ADAPTIVE_BUFFER=true` |
| Верхняя граница буфера | `MAX_BUFFER_PCT` | 0.25 | Активна при `ADAPTIVE_BUFFER=true` |
| Минимум машин | `MIN_TRUCKS` | 1 | Минимум машин при ненулевом прогнозе |
| Шагов прогноза | `FORECAST_STEPS` | 10 | 10 × 30 мин = 5 часов |
| Интервал шага | `STEP_INTERVAL_MINUTES` | 30 | Длительность одного шага |
| Окно истории | `HISTORY_WINDOW` | 288 | Наблюдений для feature engineering (288 × 30 мин ≈ 6 дней) |
| Версия модели | `MODEL_VERSION` | `v1` | Идентификатор для трекинга |
| Mock-режим | `MOCK_MODE` | `0` | Если `1` — синтетический fallback prediction-service при отсутствии артефактов модели (только для локальной разработки) |
...
### 3. Буфер на ошибку прогноза

**Допущение по умолчанию:** Фиксированный буфер 10% линейно увеличивает прогнозируемый объём.

**Обоснование:** Компенсирует ошибку модели (WAPE ~ 0.29). Фиксированный процент — простой и предсказуемый подход.

**Адаптивный режим (`ADAPTIVE_BUFFER=true`):** буфер становится плавающим между `MIN_BUFFER_PCT` (0.05) и `MAX_BUFFER_PCT` (0.25) и масштабируется по неопределённости прогноза. Включается через `.env` без пересборки.

**Путь развития:** буфер, выводимый из исторической ошибки на уровне конкретного склада или времени суток.
