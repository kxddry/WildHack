"""DatasetBuilder — team-track feature engineering pipeline.

Copied from experiments/core/data.py with ONE patch:
  build_train_test() accepts ``extra_numeric_features: list[str] | None``
  so that kaggle-style features (target_same_slot_*, momentum_*, etc.)
  added to the raw DataFrame by add_kaggle_features() are registered
  and included in the final feature set.

The upstream experiments/core/data.py is NOT modified (user policy).
"""

import numpy as np
import pandas as pd

from team_pipeline.features import TimeSeriesFeatureBuilder
...
class DatasetBuilder:
    ...
    def build_train_test(
        ...
        statistics: tuple[str, ...] = ('mean', 'std'),
        # --- PATCH: not in experiments/core/data.py ---
        ...
        # --- PATCH: register extra columns added by add_kaggle_features() ---
        if extra_numeric_features:
            present = [f for f in extra_numeric_features if f in feature_history.columns]
            self._register_numeric_features(present)
