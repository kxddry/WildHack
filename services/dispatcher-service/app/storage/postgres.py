    ...
    async with engine.connect() as conn:
        result = await conn.execute(
            text(
                ...
                "fs.latest_forecast_at, "
                "COALESCE(ts.upcoming_trucks, 0) AS upcoming_trucks "
                "FROM warehouses w "
                "LEFT JOIN ("
                "    SELECT warehouse_id, COUNT(DISTINCT route_id) AS route_count "
                "    FROM routes "
                "    GROUP BY warehouse_id"
                ") rc ON rc.warehouse_id = w.warehouse_id "
                "LEFT JOIN ("
                "    SELECT warehouse_id, MAX(created_at) AS latest_forecast_at "
                "    FROM forecasts "
                "    GROUP BY warehouse_id"
                ") fs ON fs.warehouse_id = w.warehouse_id "
                "LEFT JOIN ("
