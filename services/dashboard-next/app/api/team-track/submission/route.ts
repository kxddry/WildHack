...
export async function POST(request: NextRequest) {
  ...
  const target = `${serviceBaseUrl("retraining")}/team-track/submission${suffix}`;
    ...
    return Response.json(
      {
        error:
          error instanceof Error
            ? `Upstream Team Track submission failed: ${error.message}`
            : "Upstream Team Track submission failed",
      },
      { status: 502 }
    );
  }
